...
################################################################################
## conditional likelihood of y given a, b, sigma.y: lambda for m0 + lambda*m1 ##
################################################################################

y.likelihood.ab <- function(ab, y, Z.0, Z.1, sigma.y, lambda) {
	n <- length(y)
	a <- ab[1:dim(Z.0)[2]]    # length of a for parameter a0, a1...
	b <- ab[(dim(Z.0)[2] + 1):(dim(Z.0)[2] + dim(Z.1)[2])]  # length of b for b0, b1 
...
#####################################################################################
## compute conditional likelihood of S given gamma, a, sigma.s: gamma for S with a, U is vector of random ai ##
#####################################################################################
...
######################################################
## sample a and b, compute all the necessary vector: for numeric integration. mean approach to Expected value ##
######################################################
...
generate.ab <- function(n.sample, alpha, beta, cov.a, cov.b) {
	...
	ab <- mvrnorm(n.sample, mu = mean.ab, Sigma = cov.ab)  ## generate a, b randomly for n.sample times.
...
generate.quant <- function(data, sample.ab, alpha, beta, gamma, sigma.s, sigma.y) {
	...
	lambda <- data[1:n.mes, 7]  # med use indicator
...
Hessian.type.1 <- function(data.obj, data.factor, gradient, sample.ab, cov.a, cov.b, sigma.s) {
    ...
    ## revise here when change model
	factor.beta.beta.2 <- matrix(NA, 6, 6)
	factor.beta.beta.2[1:3, 1:3] <- inv.cov.b[1, 1]
	factor.beta.beta.2[4:6, 4:6] <- inv.cov.b[2, 2]
	factor.beta.beta.2[1:3, 4:6] <- inv.cov.b[1, 2]
	factor.beta.beta.2[4:6, 1:3] <- inv.cov.b[2, 1]
	V.a.1 <- cbind(1, a)
	V.a.2 <- cbind(1, a)
...
NR.share.parameter <- function(data, n.iter, n.sample, alpha.int, beta.int, gamma.int, cov.a, cov.b, sigma.s, sigma.y, max.loglik = T, trace = T) {
    ...
    ## revise here when change model
	beta.item <- paste(rep("beta", length(beta.int)), c("00", "01", "02", "10", "11", "12"))
...
fit.lme.sp <- function(data, n.iter, n.sample, max.loglik = T, trace = T) {
	...
	lm.b.a.0 <- lm(b.random[, 1] ~ a.random)
	lm.b.a.1 <- lm(b.random[, 2] ~ a.random)
...
fit.lme.sp.fixint <- function(data, n.iter, n.sample, max.loglik = T, trace = T) {
    ## revise here when change model
	alpha.int <- c(15.956, -2.049)
	beta.int <- c(20.898, -0.608, 3.499, -2.019, -0.063, -1.494)
	gamma.int <- c(15.866, -0.496)
	cov.a.hat <- matrix(c(45.99808, -3.726761, -3.726761, 1.378579), 2, 2)
	cov.b.hat <- matrix(c(53.46327, -8.115119, -8.115119, 5.872233), 2, 2)
	sigma.y.hat <- 4.205
	sigma.s.hat <- 1.275
	if (trace) {
		cat("cov.a", cov.a.hat, "cov.b", cov.b.hat, "sigma.s", sigma.s.hat, "sigma.y", sigma.y.hat, "\n")
    ...
    ## fit the shared-parameter model
	fm.sp <- NR.share.parameter(data = data, n.iter = n.iter, n.sample = n.sample, alpha.int = alpha.int, beta.int = beta.int, 
                gamma.int = gamma.int, cov.a = cov.a.hat, cov.b = cov.b.hat, sigma.s = sigma.s.hat, sigma.y = sigma.y.hat, max.loglik = max.loglik, trace = trace)
	return(fm.sp)
}
...
enrichd.analysis <- function(data, min.mes, n.iter, n.sample, max.loglik = T, trace = T) {
	result <- array(NA, dim = c(n.iter + 1, 11, length(min.mes)))
	name.iter <- as.character(0:n.iter)
	alpha.item <- paste(rep("alpha", 2), as.character(0:1))
    ## revise here when change model
	beta.item <- paste(rep("beta", 6), c("00", "01", "02", "10", "11", "12"))
...
enrichd.bootstrap.CI <- function(data, min.mes, n.bts, n.iter, n.sample, max.loglik = T, trace = T) {
    ## revise here when change model
	result.bts <- array(NA, dim = c(1, 11, n.bts))
...
SPM.full <- function(data=BDIdata, min.mes=5, n.boot=1000, n.iter=10, n.sample= 10000, max.loglik = T, trace = T) {
  ...
  for (i in 1:length(medall.id)) {
    ...
    medall.sp[1:n.mes, 7, i] <- lambda                             # vector delta_ij  of n.mes long
 	...
 	result.bts <- array(NA, dim = c(1, 11, n.boot))
	data <- medall.sp[, , medall.sp[2, 8, ] >= min.mes]   # depends on 4 visit or 5 visits.  557 IDs or 593 IDs
	n.sub <- dim(data)[3]
	n.bts.eff <- 1
	cat("Bootstrap ")
